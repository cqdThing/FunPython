...
FAST_DROP_SPEED = 10  # Speed when the Down key is pressed for faster drop
...
class Piece:
    def __init__(self, shape, color):
        ...
        self.held = False  # Check if piece is held or not
...
class Tetris:
    def __init__(self, root):
        ...
        self.next_piece = self.new_piece()  # Add next piece
        self.held_piece = None  # For holding a piece
        ...
        # Add score and next piece label
        self.score_label = tk.Label(root, text=f"Score: {self.score}", font=("Helvetica", 12))
        self.score_label.pack()
        self.next_piece_label = tk.Label(root, text="Next Piece:", font=("Helvetica", 12))
    ...
    def draw_board(self):
        ...
        while self.valid_move(0, 1, ghost_y):
            ghost_y += 1
        for y, row in enumerate(self.current_piece.shape):
            for x, cell in enumerate(row):
                if cell == 1:
                    self.canvas.create_rectangle(
                        (self.current_piece.x + x) * BLOCK_SIZE, 
                        ghost_y * BLOCK_SIZE + y * BLOCK_SIZE,
                        (self.current_piece.x + x + 1) * BLOCK_SIZE, 
                        ghost_y * BLOCK_SIZE + (y * BLOCK_SIZE) + BLOCK_SIZE,
    ...
    def draw_next_piece(self):
        """Draw the next piece in the next piece preview"""
        self.next_piece_label.config(text="Next Piece:")
        for y, row in enumerate(self.next_piece.shape):
            for x, cell in enumerate(row):
                if cell == 1:
                    self.canvas.create_rectangle(
                        (BOARD_WIDTH + x + 1) * BLOCK_SIZE, (y + 1) * BLOCK_SIZE,
                        (BOARD_WIDTH + x + 2) * BLOCK_SIZE, (y + 2) * BLOCK_SIZE,
                        fill=self.next_piece.color, outline='black'
                    )

    def valid_move(self, dx, dy, y_offset=None):
        """Check if moving the piece by dx, dy is valid"""
        y_offset = y_offset or self.current_piece.y
        for y, row in enumerate(self.current_piece.shape):
            for x, cell in enumerate(row):
                if cell == 1:
    ...
    def handle_key(self, event):
        ...
        elif event.keysym == 'h':  # Hold piece
            if not self.current_piece.held:
                if self.held_piece is None:
                    self.held_piece, self.current_piece = self.current_piece, self.new_piece()
                else:
                ...
                self.current_piece.pixel_y = 0
                self.current_piece.held = True
        elif event.keysym == 'p':  # Pause
            self.paused = not self.paused
            if self.paused:
                self.canvas.create_text(CANVAS_WIDTH // 2, CANVAS_HEIGHT // 2, text="Paused", font=("Helvetica", 30), fill="blue")
        elif event.keysym == 'space':  # Drop piece to bottom
